...
namespace Warshti.Panel.Areas.Application.Controllers
{
    [Area("Application")]
    [Authorize]
    public class FaqController : Controller
    {
        ...
        public IActionResult LoadData()
        {
            ...
            var tempData = _context.Faqs.AsQueryable();
            ...
            {
                tempData = tempData.Where(m => m.Question.Contains(searchValue.ToUpper()) ||
                                               m.Answer.Contains(searchValue.ToUpper()) );
            }
        ...
        {
            var temp = _context.Faqs.FirstOrDefault(p => p.Id == id);
            if (temp == null)
            {
                return NotFound();
            }

            var model = new FaqUpdateViewModel()
            {
                Id = temp.Id,
        ...
        public async Task<IActionResult> Update(FaqUpdateViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var temp = _context.Faqs.FirstOrDefault(p => p.Id == model.Id);
                if (temp == null)
                {
                    return NotFound();
                }
